...
class LoadBalancer (EventMixin):
  ...
  def __init__ (self, connection):
    ...
    self.servers = [
      self.Server('10.0.0.5', '00:00:00:00:00:05', 2),
      self.Server('10.0.0.6', '00:00:00:00:00:06', 3)]
    self.last_server = 0
    log.info("Init function end")
  ...
  def handle_request (self, packet, event):
    ...
    msg.actions.append(of.ofp_action_nw_addr.set_src('10.0.0.10'))
    
    msg.actions.append(of.ofp_action_dl_addr.set_dst(server.mac))
    msg.actions.append(of.ofp_action_dl_addr.set_dst('00:00:00:00:00:FE'))
  ...
  def _handle_PacketIn (self, event):
    ...
    if packet.type == packet.LLDP_TYPE or packet.type == packet.IPV6_TYPE:
      log.info("Packet type lldp ipv6")
      ...
      if packet.next.protodst != LOAD_BALANCER_IP:
        log.info("proto dat: %s",packet.next.protodst)
        log.info(packet.next.opcode)
        log.info(arp.REPLY)
        
        return

      log.info("Receive an ARP request")
      log.info(packet.next.hwsrc)
      log.info("proto dat: %s",packet.next.protodst)
